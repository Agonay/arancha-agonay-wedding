...
Music tables (`012`) live — `music_playlist` empty (couple hasn't added songs yet), `song_proposals` had 5 test proposals pending/queued. `feature_flags` seeded with `wedding_day_mode=false` and `music_proposals_open=false`. Spotify API credentials configured (`SPOTIFY_CLIENT_ID`, `SPOTIFY_CLIENT_SECRET`). **FIXED 2026-08-31**: DJ queue went blank on page reload because queries used non-existent `guests.full_name` instead of `guests.display_name` — the PostgREST select errored, `data` came back `null`, and `(rawProposals || [])` mapped to `[]`. Realtime inserts still worked because they read `payload.new` directly. Now fixed; also added a "Recargar" (refresh) button as a realtime fallback, and album-art placeholders/fallbacks across DJ queue, playlist manager, and guest tabs.

---
*Last updated: 2026-08-31 — Fixed DJ queue blank-on-reload (root cause: `guests.full_name` doesn't exist → used `guests.display_name`; query was silently failing so `data` was null) + added refresh button and album-art fallbacks. Phase 10b Music system complete (playlist manager, DJ queue with Supabase Realtime, guest proposal form with Spotify/Deezer search, wedding day tab system). Production LIVE on aranzazuagonay.es. Any session that finishes work MUST refresh the Status/Checklist sections above.*
