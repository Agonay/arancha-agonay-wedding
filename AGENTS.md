...
- [x] RSVP reset: individual "Limpiar RSVP" (delete the rsvp row, guest returns to pending) with a custom confirmation modal (`ConfirmModal` in `src/components/ui/ConfirmModal.tsx`) — available on the guest edit page (`onDeleteRsvp`), the guest table (Eraser icon per row with RSVP), and the /admin/rsvps overview (per-row Trash icon via `RsvpRowActions.tsx`). Server action `deleteRsvp(guestId)` in `src/features/rsvp/actions.ts` deletes by `guest_id` and revalidates `/admin/guests`, `/admin/rsvps`, `/admin/dashboard`. No schema change.
